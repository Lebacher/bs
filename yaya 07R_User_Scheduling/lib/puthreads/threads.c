...
void threads_exit(void* result) {
    ...
    QUEUE** check_feedback = &feedback;
    do {
        if ((*check_feedback)->head != NULL) {
            break;
        }
        check_feedback = &((*check_feedback)->next);
    } 
    while ((*check_feedback) != NULL);
    running = NULL;

    if ((*check_feedback)->head == NULL ||
        ((running = queue_dequeue(*check_feedback)) == NULL)) {
        abort();
    }
...
static void handle_sigprof(int signum, siginfo_t* nfo, void* context) {
    ...
    while (*check_feedback != NULL){
        thread_counter += queue_size(*check_feedback);
        check_feedback = &((*check_feedback)->next);
    }

    //exit if no thread is ready for execution
    if (thread_counter == 0 && running == NULL){
        _exit(EXIT_SUCCESS);
    }
    ...
    int current_queue = 0;
    while ((*check_feedback)->next != NULL) {
        check_feedback =&((*check_feedback)->next);
        current_queue++;
    ...
    if (rescheudle){
        int depth_run = running->feedback_depth;
        if (depth_run != FEEDBACK_DEPTH - 1){
            depth_run--;
        }
        int enqueue_result = queue_enqueue(*check_feedback, running);
        if (enqueue_result != 0) {
            abort();
        }
    }
    ...
    if ((*check_feedback)->head == NULL ||
        (running = queue_dequeue(*check_feedback))) {
        abort();
    }
